import Link from "next/link";

export default function HomeServices() {
    return (
        <section>
         <div className="tp-service-area service-space-bottom pt-40 pb-60 ">
            <div className="container">
               <div className="row">
                  <div className="col-xl-5 col-lg-5 left-sticky wow tpfadeLeft" data-wow-duration=".7s"
                     data-wow-delay=".5s">
                     <div className="service-section-wrapper ">
                        <div className="tp-service-section-title-box">
                           <span className="tp-section-subtitle tp-yellow-color text-capitalize ">Elevate Your Brand with
                              Our
                              Expertise</span>
                           <h2 className="pb-3 text-capitalize">your trusted partner for digital branding excellence.</h2>
                           <p className="mb-24">Welcome to <strong className="content-color text-capitalize">Inizio
                                 Solutions</strong>, your trusted
                              partner for
                              premium Apparel Branding Services in
                              the USA. Our mission is to transform your creative
                              ideas from logos to complex
                              illustrations into stunning, production-ready digital artwork. Since 2015, we've
                              specialized in providing expert <strong
                                 className="content-color text-capitalize">Digitizing</strong> and
                              <strong className="content-color text-capitalize">Vector Art Conversion</strong> for brands,
                              ensuring
                              every stitch-out and print job is flawless. When you choose us, you are guaranteed fast
                              turnaround and the highest digital service
                              quality.

                           </p>
                           <div className="tp-service-content mb-20">
                              <Link className="tp-btn mt-10 text-capitalize" href="/services">See all Services</Link>
                           </div>

                        </div>
                     </div>
                  </div>
                  <div className="col-xl-7 col-lg-7">
                     <div className="row service-space-left">
                        <div className="col-12 wow tpfadeUp" data-wow-duration=".7s" data-wow-delay=".7s">
                           <div className="tp-service-item mb-20">
                              <div className="d-flex justify-content-between ">
                                 <div className="tp-service-item__content">
                                    <h3 className="tp-service-sm-title"><Link href="/services/digitizing">Embroidery Digitizing</Link>
                                    </h3>
                                 </div>
                                 <div className="tp-service-item__button-link">
                                    <Link href="/services/digitizing">
                                       <span>
                                          <svg width="34" height="16" viewBox="0 0 34 16" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                             <path
                                                d="M33.7071 8.70711C34.0976 8.31659 34.0976 7.68342 33.7071 7.2929L27.3431 0.928935C26.9526 0.53841 26.3195 0.53841 25.9289 0.928934C25.5384 1.31946 25.5384 1.95262 25.9289 2.34315L31.5858 8L25.9289 13.6569C25.5384 14.0474 25.5384 14.6805 25.9289 15.0711C26.3195 15.4616 26.9526 15.4616 27.3431 15.0711L33.7071 8.70711ZM-8.74228e-08 9L33 9L33 7L8.74228e-08 7L-8.74228e-08 9Z"
                                                fill="currentColor" />
                                          </svg>
                                       </span>
                                       Read More
                                    </Link>
                                 </div>
                              </div>
                              <div className="tp-service-item__description">
                                 <p>Convert your logos and designs into perfectly optimized, production-ready embroidery
                                    files (DST, PES, etc.). We ensure clean stitch flow and balanced density for all
                                    apparel, including caps, uniforms, and jackets.</p>
                              </div>
                           </div>

                        </div>
                        <div className="col-12 wow tpfadeUp" data-wow-duration=".7s" data-wow-delay=".7s">
                           <div className="tp-service-item sv-color-blue-one mb-20">
                              <div className="d-flex justify-content-between ">
                                 <div className="tp-service-item__content">
                                    <h3 className="tp-service-sm-title"><Link href="/services/vector">Vector Art Conversion</Link>
                                    </h3>
                                 </div>
                                 <div className="tp-service-item__button-link">
                                    <Link href="/services/vector">
                                       <span>
                                          <svg width="34" height="16" viewBox="0 0 34 16" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                             <path
                                                d="M33.7071 8.70711C34.0976 8.31659 34.0976 7.68342 33.7071 7.2929L27.3431 0.928935C26.9526 0.53841 26.3195 0.53841 25.9289 0.928934C25.5384 1.31946 25.5384 1.95262 25.9289 2.34315L31.5858 8L25.9289 13.6569C25.5384 14.0474 25.5384 14.6805 25.9289 15.0711C26.3195 15.4616 26.9526 15.4616 27.3431 15.0711L33.7071 8.70711ZM-8.74228e-08 9L33 9L33 7L8.74228e-08 7L-8.74228e-08 9Z"
                                                fill="currentColor" />
                                          </svg>
                                       </span>
                                       Read More
                                    </Link>
                                 </div>
                              </div>
                              <div className="tp-service-item__description">
                                 <p>We convert any raster image (JPG, PNG) into clean, scalable, print-ready vector
                                    artwork. Perfect for all printing techniques: screen printing, DTG, laser etching,
                                    and large format vinyl.</p>
                              </div>
                           </div>

                        </div>
                        <div className="col-12 wow tpfadeUp" data-wow-duration=".7s" data-wow-delay=".7s">
                           <div className="tp-service-item sv-color-blue-two mb-20">
                              <div className="d-flex justify-content-between ">
                                 <div className="tp-service-item__content">
                                    <h3 className="tp-service-sm-title"><Link href="/services/custom">Premium Custom
                                          Patches</Link>
                                    </h3>
                                 </div>
                                 <div className="tp-service-item__button-link">
                                    <Link href="/services/custom">
                                       <span>
                                          <svg width="34" height="16" viewBox="0 0 34 16" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
                                             <path
                                                d="M33.7071 8.70711C34.0976 8.31659 34.0976 7.68342 33.7071 7.2929L27.3431 0.928935C26.9526 0.53841 26.3195 0.53841 25.9289 0.928934C25.5384 1.31946 25.5384 1.95262 25.9289 2.34315L31.5858 8L25.9289 13.6569C25.5384 14.0474 25.5384 14.6805 25.9289 15.0711C26.3195 15.4616 26.9526 15.4616 27.3431 15.0711L33.7071 8.70711ZM-8.74228e-08 9L33 9L33 7L8.74228e-08 7L-8.74228e-08 9Z"
                                                fill="currentColor" />
                                          </svg>
                                       </span>
                                       Read More
                                    </Link>
                                 </div>
                              </div>
                              <div className="tp-service-item__description">
                                 <p>Design and manufacture high-quality custom patches in any style: embroidered, PVC,
                                    woven, or chenille. Ideal for brands needing fine detail, durability, and standout
                                    uniform branding.</p>
                              </div>
                           </div>

                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </section>
    )
}